...
function ProfileScreen() {
	...
	const { id, matric_number, department, faculty, level } = user?.student;
		...
		<View className="flex-1">
			...
			<Body style={"bg-gray-50"}>
				<Card style="my-2 items-center justify-center">
					<Image source={{ uri: profile_image }} className="rounded-full w-24 h-24 my-4" />
				...
				<Card style="my-2" title="Health Center Info">
					<Text className="text-gray-500 mt-1">
						Health Centre id: <Text className="text-black font-medium">{user.id.toUpperCase()}</Text>
					</Text>
